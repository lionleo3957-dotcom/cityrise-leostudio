...
function checkBank(){
  ...
  if(crash < 0.2){ logMessage("Leobank temporarily unavailable! ❌"); }
  else { logMessage(Current Balance: ✦${LUMA}); }
}
...
function updatePhone(){
  ...
  document.getElementById("leobank").innerText = Leobank Balance: ✦${LUMA};
}
...
function advanceTime(minToAdd){
  ...
  if(hours >= 24){ hours-=24; day++; LUMA += 10; logMessage("Parents gave ✦10 LUMA"); }
  if(hours >= 20){ logMessage("Taxi work must end! Go home 🏠"); }
  updatePhone();
}
...
function taxiCustomer(){
  ...
  if(type==="Bank"){
    ...
    else{ LUMA += earn; logMessage(Customer paid ✦${earn} via Bank ✅); }
  } else { LUMA += earn; logMessage(Customer paid ✦${earn} in Cash ✅); }
  skills["Driving"] += 2;
  showSkills();
  updatePhone();
}
...
updatePhone();
logMessage("Game Started! Day 1, 8:00 AM. ✦10 from parents.");
